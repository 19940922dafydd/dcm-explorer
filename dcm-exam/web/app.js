/**
 * DCM 资源管家 - PyWebView Frontend Logic
 */
...
window.addEventListener('pywebviewready', function() {
    ...
    selectFolderBtn.addEventListener('click', async () => {
        try {
            ...
            window.pywebview.api.start_scan({ rootPath: folderPath, startTs, endTs });
    ...
    analyzeBtn.addEventListener('click', () => {
        ...
        window.pywebview.api.analyze_dicom(paths);
    });

    // 导出逻辑
    exportBtn.addEventListener('click', async () => {
        // PyWebView implementation of export can be added here
        alert("基于 Python 环境的导出功能待接入");
    });
...
// 接收主进程扫描数据
window.onScanResultsBatch = function(batch) {
    allFiles.push(...batch);
    applyFilterAndRender();
};

window.onScanProgressTotal = function(total) {
    scanProgress.value = (total % 100);
};

window.onScanFinished = function(finalTotal) {
...
// 分析进度响应
window.onAnalyzeResultsBatch = function(batch) {
    // 映射新的dicom数据
...
window.onAnalyzeFinished = function(totalAnalyzed) {
...
// 解码错误的GBK中文字符 (The python backend now does proper decoding, but we leave this here if needed, or we can trust python)
// Actually python returns utf-8 correct strings using pydicom!
